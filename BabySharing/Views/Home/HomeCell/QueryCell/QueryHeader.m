...
- (void)setTime:(NSDate*)date {
    ...
    _timeLabel.frame = CGRectMake(USER_IMG_MARGIN + USER_IMG_WIDTH + USER_IMG_MARGIN, 28, TIME_ICON_WIDTH + label.frame.size.width, label.frame.size.height);
}
...
- (void)setRoleTag:(NSString *)role_tag {
    ...
    _userRoleTagBtn.frame = CGRectMake(_userNameLabel.center.x + _userNameLabel.frame.size.width / 2 + TAG_2_NAME_MARGIN, 4, label.frame.size.width + 10 + ROLE_TAG_MARGIN, label.frame.size.height + 2 * ROLE_TAG_MARGIN);
}
